...
print("Успешное подключение БД - Регистрация")
...
class Ui_RegWindow(object):
    ...
    def setupUi(self, RegWindow):
        ...
        RegWindow.resize(800, 840)
        RegWindow.setMinimumSize(QtCore.QSize(800, 840))
        RegWindow.setMaximumSize(QtCore.QSize(800, 840))
        icon = QtGui.QIcon()
...
##############
        ...
        self.lineEdit_7.textChanged.connect(self.login)
...
###############
        ...
        self.pushButton_2.clicked.connect(self.window_login)
        self.pushButton_2.clicked.connect(RegWindow.close)
...
"}")
    ...
    def insert_data(self):
        ...
        date_born = self.lineEdit_6.text()
        ...
        if len(date_born) == 0:
            self.label.setText("Введите дату рождения!")
            return
        ...
        mycursor.execute(f'SELECT fio FROM `user` WHERE (fio = "{fio}") OR (login = "{login}") OR (phone = "{phone}") OR (phone = "{email}")')
        if mycursor.fetchone() is None:
            mycursor.execute(f'INSERT INTO `user` (fio, phone,  email, data_born, login, password) VALUES ("{fio}", "{phone}", "{email}","{date_born}", "{login}", "{password}")')
            self.label.setText("Успешно добавлено!")
            mydb.commit()
        else:
            self.label.setText("Такой аккаут уже есть!")
...
#################################
    ...
    def retranslateUi(self, RegWindow):
        ...
        self.label_5.setText(_translate("RegWindow", "Регистрация"))
        self.label_2.setText(_translate("RegWindow", "ФИО"))
        self.label_7.setText(_translate("RegWindow", "Телефон"))
        self.label_8.setText(_translate("RegWindow", "Email"))
        self.label_9.setText(_translate("RegWindow", "Дата рождения"))
        self.label_10.setText(_translate("RegWindow", "Логин"))
        self.label_3.setText(_translate("RegWindow", "Пароль"))
        # self.label.setText(_translate("RegWindow", "Сообщение!"))
        self.pushButton_3.setText(_translate("RegWindow", "Создать"))
        self.pushButton_2.setText(_translate("RegWindow", "Назад"))
